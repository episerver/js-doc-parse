define([ './env', './node!fs', './node!path' ], function (env, fs, pathUtil) {
	...
	function getModuleIdFromPath(/**string*/ path) {
		...
		for (var packageName in packages) {
			if (packages.hasOwnProperty(packageName)) {
				...
				// "/foo/barbaz"
				if (pathPrefix.charAt(pathPrefix.length - 1) !== pathUtil.sep) {
					pathPrefix += pathUtil.sep;
				}

				if (path.indexOf(pathPrefix) === 0) {
					moduleId = packageName + pathUtil.sep + path.substr(pathPrefix.length);
					match = true;
					break;
				}
			}
		...
		return moduleId.replace(/\.js$/g, '').split(pathUtil.sep).join('/');
	}
	...
	File.prototype = {
		...
		resolveRelativeId: function (/**string*/ moduleId) {
			...
			moduleId = moduleId.split(pathUtil.sep).join('/');
			if (moduleId.charAt(0) === '.') {
				moduleId = this.moduleId.replace(/\/[^\/]+$/, '/') + moduleId;
			}

			moduleId = pathUtil.normalize(moduleId).split(pathUtil.sep).join('/');
